      ...
      <div className="min-h-screen bg-surface pb-16">
        <div className="bg-white border-b border-black/[0.05] px-4 md:px-6 py-4">
          <div className="max-w-3xl mx-auto">
            <h1 className="text-base font-bold text-ink">Programas Privados Cortos</h1>
            <p className="text-xs text-ink-muted mt-0.5">Genera cotización y contrato para el prospecto</p>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 md:px-6 py-6 space-y-6">
          ...
          <div className="portal-card p-5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-sm font-bold text-ink">Contratos en blanco para imprimir</h2>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <select value={blankProgram} onChange={(e) => setBlankProgram(e.target.value as ProgramKey)}
                className="portal-select flex-1">
              ...
              return (
                <div key={label} className="flex items-center flex-1 last:flex-none">
                  <div className="flex flex-col items-center">
                    <div className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${done ? 'bg-ink text-white' : active ? 'bg-ink text-white' : 'bg-surface text-ink-muted'}`}>
                      {done ? <Check className="h-4 w-4" /> : n}
                    </div>
                    <span className={`text-xs mt-1 font-medium ${active ? 'text-ink' : done ? 'text-ink' : 'text-ink-muted'}`}>{label}</span>
                  </div>
                  {i < steps.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-1 mb-5 transition-colors ${done ? 'bg-ink' : 'bg-black/[0.08]'}`} />
                  )}
                </div>
              )
            })}
          ...
          {step === 1 && (
            <div className="portal-card p-6">
              <h2 className="text-sm font-bold text-ink mb-4">¿Qué programa desea el prospecto?</h2>
              <div className="space-y-3">
                {(Object.entries(PROGRAMS) as [ProgramKey, { label: string; equipCost: number; noEquipment: boolean }][]).map(([key, prog]) => (
                  <button
                    ...
                    className={`w-full text-left border-2 rounded-xl2 p-4 transition-all ${program === key ? 'border-ink bg-surface' : 'border-black/[0.06] hover:border-black/[0.12]'}`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-ink">{prog.label}</p>
                        <p className="text-xs text-ink-muted mt-0.5">36 horas de contacto</p>
                        <div className="flex gap-4 mt-2 text-xs text-ink-muted">
                          <span>Programa: <span className="font-medium">$495.00</span></span>
                          {prog.noEquipment
                            ? <span className="text-ink-muted italic">Sin equipo institucional</span>
                            : <span>Equipo: <span className="font-medium">{fmt(prog.equipCost)}</span></span>
                          }
                        </div>
                      </div>
                      <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${program === key ? 'border-ink bg-ink' : 'border-black/[0.15]'}`}>
          ...
          {step === 2 && (
            <div className="portal-card p-6">
              <h2 className="text-sm font-bold text-ink mb-4">Información del Estudiante</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className={labelCls}>Nombre completo <span className="text-accent">*</span></label>
                ...
                <div>
                  <label className={labelCls}>Teléfono <span className="text-accent">*</span></label>
                ...
                <div>
                  <label className={labelCls}>Fecha de inicio <span className="text-ink-muted font-normal">(opcional)</span></label>
                ...
                <div>
                  ...
                  <select className="portal-select" value={student.horario} onChange={(e) => setSI('horario', e.target.value)}>
                ...
                <div>
                  ...
                  <select className="portal-select" value={student.modalidad} onChange={(e) => setSI('modalidad', e.target.value)}>
                ...
                <div className="sm:col-span-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={student.menor} onChange={(e) => setSI('menor', e.target.checked)} className="h-4 w-4 rounded border-black/[0.15]" />
                    <span className="text-sm text-ink">El estudiante es menor de 18 años (requiere firma de padre/madre/tutor)</span>
                  </label>
                </div>
                ...
                {student.menor && (
                  <>
                    <div>
                      <label className={labelCls}>Nombre del padre/madre/tutor <span className="text-accent">*</span></label>
                    ...
                    <div>
                      <label className={labelCls}>Teléfono del tutor <span className="text-accent">*</span></label>
              ...
              <div className="mt-5 flex justify-between">
                <button onClick={() => setStep(1)} className="portal-btn-ghost">
          ...
          {step === 3 && program && (
            <div className="space-y-4">
              {/* Program payment */}
              <div className="portal-card p-6">
                <h2 className="text-sm font-bold text-ink mb-1">Sección A — Plan de Pago del Programa</h2>
                <p className="text-xs text-ink-muted mb-4">Cargo de admisión de $25.00 se cobra por separado en todos los casos.</p>
                <div className="space-y-3">
                  {([
                    ...
                    <button key={key} onClick={() => setProgPlan(key)}
                      className={`w-full text-left border-2 rounded-xl2 p-4 transition-all ${progPlan === key ? 'border-ink bg-surface' : 'border-black/[0.06] hover:border-black/[0.12]'}`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className={`h-5 w-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${progPlan === key ? 'border-ink bg-ink' : 'border-black/[0.15]'}`}>
              ...
              {!PROGRAMS[program].noEquipment ? (
                <div className="portal-card p-6">
                  <h2 className="text-sm font-bold text-ink mb-1">Sección B — Equipo / Materiales / Kit</h2>
                  <p className="text-xs text-ink-muted mb-4">Costo base del equipo para {PROGRAMS[program].label}: {fmt(PROGRAMS[program].equipCost)}</p>
                  {PROGRAMS[program].materials && (
                    <div className="mb-4 rounded-xl2 bg-surface p-3">
                      <p className="text-xs font-semibold text-ink-muted mb-2">Materiales incluidos</p>
                      <div className="space-y-1">
                        {PROGRAMS[program].materials!.map((m) => (
                          <div key={m.name} className="flex justify-between text-xs text-ink-muted">
                            <span>{m.name}</span><span className="font-medium">{fmt(m.price)}</span>
                          </div>
                        ))}
                      </div>
                      <p className="text-[11px] text-ink-muted mt-2">Precios sujetos a cambios según el mercado.</p>
                    </div>
                  )}
                  <div className="space-y-3">
                    {([
                      ...
                      <button key={key} onClick={() => setEqPlan(key)}
                        className={`w-full text-left border-2 rounded-xl2 p-4 transition-all ${eqPlan === key ? 'border-ink bg-surface' : 'border-black/[0.06] hover:border-black/[0.12]'}`}
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <p className="font-semibold text-ink text-sm">{title}</p>
                          ...
                          <div className={`h-5 w-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${eqPlan === key ? 'border-ink bg-ink' : 'border-black/[0.15]'}`}>
                ...
                <div className="bg-surface rounded-neu p-5">
                  <p className="text-sm text-ink font-medium">Este programa no incluye equipo o kit institucional.</p>
                  <p className="text-xs text-ink-muted mt-1">Facturación a Planes Médicos no requiere equipo — solo aplica el pago del programa y el cargo de admisión.</p>
                </div>
              )}
              ...
              {prices && (
                <div className="bg-surface rounded-neu p-5">
                  <h3 className="text-sm font-bold text-ink mb-3">Resumen de Precios — Escenario #{prices.scenario}</h3>
              ...
              <div className="flex justify-between">
                <button onClick={() => setStep(2)} className="portal-btn-ghost">
                  <ChevronLeft className="h-4 w-4" /> Atrás
                </button>
                <button
                  onClick={() => setStep(4)}
          ...
          {step === 4 && program && progPlan && eqPlan && prices && (
            <div className="space-y-4">
              <div className="portal-card p-6">
                <h2 className="text-sm font-bold text-ink mb-4">Resumen del Contrato</h2>
                ...
                {saveError && (
                  <div className="mt-4 p-3 rounded-xl2 bg-accent-soft flex gap-2 items-start">
                    <AlertCircle className="h-4 w-4 text-accent flex-shrink-0 mt-0.5" />
                    <p className="text-sm text-accent">{saveError}</p>
                  </div>
                )}
                {saved && (
                  <div className="mt-4 p-3 rounded-xl2 bg-surface flex gap-2 items-start">
                    <CheckCircle className="h-4 w-4 text-ink flex-shrink-0 mt-0.5" />
                    <p className="text-sm text-ink">Contrato guardado en el historial.</p>
                  </div>
                )}

                <div className="mt-6 flex justify-between items-center flex-wrap gap-3">
                  <button onClick={() => setStep(3)} className="portal-btn-ghost">
          ...
          <div className="portal-card overflow-hidden">
            <div className="px-5 py-4 border-b border-black/[0.05]">
              <h2 className="text-sm font-bold text-ink">Contratos Generados</h2>
            </div>
            {loadingContracts ? (
              ...
              <div className="portal-empty">
                <p className="text-sm text-ink-muted">Aún no has generado ningún contrato.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-black/[0.05] bg-surface">
                      <th className="text-left px-4 py-3 font-semibold text-ink-muted text-xs">Fecha</th>
                      <th className="text-left px-4 py-3 font-semibold text-ink-muted text-xs">Estudiante</th>
                      <th className="text-left px-4 py-3 font-semibold text-ink-muted text-xs hidden sm:table-cell">Programa</th>
                      <th className="text-left px-4 py-3 font-semibold text-ink-muted text-xs hidden md:table-cell">Esc.</th>
                      <th className="text-left px-4 py-3 font-semibold text-ink-muted text-xs">Total</th>
                      <th className="text-left px-4 py-3 font-semibold text-ink-muted text-xs hidden lg:table-cell">Generado por</th>
                      <th className="text-right px-4 py-3 font-semibold text-ink-muted text-xs">Ver</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-black/[0.05]">
                    {contracts.map((c) => (
                      <tr key={c.id} className="hover:bg-surface">
                        ...
                        <td className="px-4 py-3 text-xs text-ink-muted hidden sm:table-cell">{PROGRAM_LABELS[c.program] ?? c.program}</td>
                        <td className="px-4 py-3 text-xs text-ink-muted hidden md:table-cell">#{c.scenario}</td>
                        <td className="px-4 py-3 text-xs font-medium text-ink">{fmt(c.total_contract)}</td>
                        <td className="px-4 py-3 text-xs text-ink-muted hidden lg:table-cell">{c.employee_name}</td>
