...
import {
  CheckCircle2, XCircle, Clock, Download, ShieldCheck, GraduationCap, BookOpen,
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-ink-muted">Revisa el contenido subido, verifica la autorización y apruébalo para usarlo en redes, web y promociones.</p>
      </div>

      {/* Filtros */}
      <div className="portal-tabs w-fit">
        {FILTERS.map(({ key, label, Icon }) => (
          <button key={key} onClick={() => setFilter(key)}
            className={`portal-tab flex items-center gap-1.5 ${filter === key ? 'portal-tab--active' : ''}`}>
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-sm text-ink-muted py-12 justify-center"><span className="portal-spinner h-4 w-4" /> Cargando…</div>
      ) : subs.length === 0 ? (
        <div className="portal-card portal-empty">
          <ImageIcon className="h-9 w-9 text-ink-muted/50 mx-auto mb-2" />
          <p className="text-sm text-ink-muted">No hay contenido {filter !== 'todos' ? `(${filter})` : ''} por ahora.</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {subs.map((s) => (
            <div key={s.id} className="portal-card overflow-hidden flex flex-col">
              <div className="relative aspect-[4/3] bg-ink flex items-center justify-center">
                {s.url && s.kind === 'imagen'
                  ...
                  : s.url && s.kind === 'video'
                    ...
                    : <span className="text-ink-muted">{s.kind === 'video' ? <Film /> : <ImageIcon />}</span>}
              ...
              <div className="p-4 flex flex-col gap-2.5 flex-1">
                ...
                {s.caption && <p className="text-xs text-ink-muted leading-snug line-clamp-2">{s.caption}</p>}

                <div className="flex flex-col gap-0.5 text-xs">
                  <div className="flex items-center gap-1.5 text-ink-muted">
                    {s.author_type === 'profesor' ? <BookOpen className="h-3.5 w-3.5 text-accent" /> : s.author_type === 'estudiante' ? <GraduationCap className="h-3.5 w-3.5 text-accent" /> : <UserRound className="h-3.5 w-3.5 text-ink-muted" />}
                    <span className="font-semibold text-ink">{s.author_name}</span>
                    {s.author_type && <span className="text-ink-muted">· {s.author_type}</span>}
                  </div>
                  {s.author_email && (
                  ...
                  <span className="text-ink-muted pl-[19px]">{fmtDate(s.created_at)} · {fmtSize(s.size_bytes)}</span>
                </div>

                {/* Consentimiento */}
                <div className="rounded-lg bg-surface border border-black/[0.05] px-2.5 py-1.5 text-[11px] text-ink-muted flex items-start gap-1.5">
                  <ShieldCheck className="h-3.5 w-3.5 flex-shrink-0 mt-0.5" />
                  <span>
                    Autorizó uso en redes/web/promos {s.consent_version && <span className="text-ink-muted">({s.consent_version})</span>}.
                    {s.is_minor && <span className="block text-ink-muted">Menor · aval del tutor{s.guardian_name ? `: ${s.guardian_name}` : ''}.</span>}
                  </span>
                </div>

                {s.status === 'rechazado' && s.review_note && <p className="text-[11px] text-accent">Motivo: {s.review_note}</p>}

                <div className="mt-auto flex items-center gap-2 pt-1">
                  {s.status !== 'aprobado' && (
                    <button onClick={() => review(s.id, 'aprobado')} disabled={busyId === s.id}
                      className="portal-btn flex-1 gap-1.5 px-3 py-2 text-xs">
                      {busyId === s.id ? <span className="portal-spinner h-3.5 w-3.5 border-2 border-white/30 border-t-white" /> : <CheckCircle2 className="h-3.5 w-3.5" />} Aprobar
                    </button>
                  )}
                  {s.status !== 'rechazado' && (
                    <button onClick={() => review(s.id, 'rechazado')} disabled={busyId === s.id}
                      className="portal-btn-ghost gap-1.5 px-3 py-2 text-xs hover:text-accent">
                  ...
                  {s.url && (
                    <a href={s.url} target="_blank" rel="noopener noreferrer" title="Descargar / ver original"
                      className="portal-btn-ghost px-3 py-2">
                      <Download className="h-3.5 w-3.5" />
                    </a>
                  )}
                </div>
                <span className={`inline-flex items-center gap-1 text-[10px] font-bold ${s.status === 'aprobado' ? 'text-ink' : s.status === 'rechazado' ? 'text-accent' : 'text-ink-muted'}`}>
